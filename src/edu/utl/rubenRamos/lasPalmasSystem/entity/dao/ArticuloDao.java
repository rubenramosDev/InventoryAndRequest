...
public class ArticuloDao implements IArticulo {
    ...
    public ArrayList<Articulo> getAllAvailableArticulos() throws SQLException {
        ...
        try {
            ArrayList<Articulo> listArticulos = new ArrayList<>();
            String sql = "SELECT ar.id_articulo, ar.nombre, ar.precio_unitario, ar.precio_faltante, ar.cantidad, ar.imagen_path, ca.id_categoria, ca.nombre, ca.forma FROM articulos ar INNER JOIN categoria_articulos ca on ar.id_categoria_articulo = ca.id_categoria WHERE ar.cantidad> 0 AND ar.estatus=TRUE";
